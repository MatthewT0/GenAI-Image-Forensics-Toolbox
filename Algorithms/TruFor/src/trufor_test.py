...
with torch.no_grad():
    for index, (rgb, path) in enumerate(tqdm(testloader)):
        ...
        if os.path.splitext(os.path.basename(output))[1] == '':  # output is a directory            
            # Fixing filename paths 
            if isinstance(path, tuple):
                path = path[0]
            filename_out = os.path.join(output, os.path.basename(path) + '.npz')
        ...
        if not (os.path.isfile(filename_out)):
            try:
                ...
                # returns just the output map for matlab to jsondecode
                # Convert map to a list for easy conversion in MATLAB
                output_map = out_dict['map'].tolist()

                # Ensure only the JSON gets printed
                sys.stdout.write(json.dumps(output_map))
                sys.stdout.flush()  # Force immediate output
                
                # returns all outputs to a npz file

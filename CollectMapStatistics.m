function Curves = CollectMapStatistics( Options )
    ...
    % compare the mean inside and outside the masks
    ...
    % compare the median inside and outside the masks
    ...
    % remove the negative from values and concat them too same array
    MeansDiffs=abs([MeansDiffsSp MeansDiffsAu]);
    MediansDiffs=abs([MediansDiffsSp MediansDiffsAu]);
    % take only the 2nd and 98th percentile, to create a lower and upper range
    MeansRange=[prctile(MeansDiffs,2) prctile(MeansDiffs,98)];
    MediansRange=[prctile(MediansDiffs,2) prctile(MediansDiffs,98)];

    % find 500 equal steps between the lower and upper range
    MeanStep=(MeansRange(2)-MeansRange(1))/500;
    MedianStep=(MediansRange(2)-MediansRange(1))/500;
    % loop through the range in the equal steps and add all values to a new variable
    % inf is just below and just above the range, guaranteeing outliers can still be handled by the threshold
